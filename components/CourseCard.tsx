...
export const CourseCard: React.FC<CourseCardProps> = ({ 
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden hover:shadow-md transition-shadow duration-300 flex flex-col h-full relative group">
      <div className="relative h-48 cursor-pointer" onClick={() => onViewDetails && onViewDetails(course)}>
        <img 
          src={course.image} 
          alt={course.title} 
      ...
      <div className="p-6 flex-1 flex flex-col">
        <div className="flex-1">
            ...
            <h3 className="text-xl font-bold text-gray-900 mb-2 group-hover:text-indigo-600 transition-colors cursor-pointer" onClick={() => onViewDetails && onViewDetails(course)}>{course.title}</h3>
            <div 
        ...
        <div className="mt-auto pt-4 border-t border-gray-100">
            ...
            {isRegistered ? (
                    ...
                    <div className="flex gap-2">
                        {isCompleted && onViewCertificate ? (
                            <Button 
                                variant="secondary" 
                                size="sm" 
                                className="flex-1 shadow-md flex items-center justify-center" 
                            ...
                            <Button variant="outline" size="sm" className="flex-1 opacity-75 cursor-not-allowed bg-yellow-50 text-yellow-700 border-yellow-200" disabled>
                                Pending
                            </Button>
                        ) : onRequestCompletion ? (
                            <Button variant="secondary" size="sm" className="flex-1" onClick={() => onRequestCompletion(course.id)}>
                                Complete
                            </Button>
                        ) : (
                             <div className="flex-1 text-center py-2 bg-gray-50 text-gray-500 text-sm rounded-lg border border-gray-100">
                                Registered
                            </div>
                        )}
                         <Button variant="outline" size="sm" onClick={() => onViewDetails && onViewDetails(course)}>
                            Details
                        </Button>
                    </div>
                </div>
                ...
                <div className="flex gap-2">
                    <Button onClick={() => onRegister(course.id)} className="flex-1" disabled={isAdmin}>
                        {isAdmin ? 'Admin Mode' : 'Register'}
                    </Button>
                    <Button variant="outline" onClick={() => onViewDetails && onViewDetails(course)}>
                        Details
                    </Button>
                </div>
            )}
