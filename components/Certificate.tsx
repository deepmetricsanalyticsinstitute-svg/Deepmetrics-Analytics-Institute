...
export const Certificate: React.FC<CertificateProps> = ({ user, course, onClose, onUpdateCourse, allCourses }) => {
  ...
  const handleSignatureUpload = (e: React.ChangeEvent<HTMLInputElement>, targetId?: string) => {
      ...
      // Restrict to JPG, PNG, SVG as requested
  ...
  useEffect(() => {
    if (isCroppedPreview && showCropModal) {
        const newImage = generateCroppedImage();
        if (newImage) setPendingImage(newImage);
    }
  }, [removeBackground]);
                   ...
                   <div className="p-8 flex flex-col items-center gap-6 overflow-y-auto flex-1">
                       ...
                       <div className="flex gap-3 w-full justify-end pt-4 border-t border-gray-100 mt-auto items-center">
                            <div className="flex items-center gap-2 mr-auto">
                                ...
                                />
                                <label htmlFor="removeBg" className="text-sm text-gray-700 select-none cursor-pointer">Remove White Background</label>
                            </div>
